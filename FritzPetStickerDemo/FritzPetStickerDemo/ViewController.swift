...
UINavigationControllerDelegate {
  ...
  func createSticker(_ uiImage: UIImage) {
    ...
    let maskCI = CIImage(cgImage: mask.cgImage!)
    let inputCI = CIImage(cgImage: uiImage.cgImage!)
    let empty = CIImage.empty()

    guard let filter = CIFilter(name: "CIBlendWithAlphaMask") else { return }
    filter.setValue(inputCI, forKey: "inputImage")
    ...
    DispatchQueue.main.async {
      self.imageView.image = UIImage(cgImage: maskedImage)
    }
  }
}
